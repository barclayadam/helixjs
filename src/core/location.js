hx.singleton('$location', ['$bus', '$log'], function($bus, $log) {
    var location = {},
    ...
    /** 
      ...
      bookmarkable.
    */
    var uri = location.uri = ko.observable();
    ...
    location.routePath = ko.computed({
        ...
        */
        write: function (newPath, options) {
            ...
            $bus.publish('urlChanged:internal', {
                url: _getFragment(),
    ...
    */
    location.initialise = function () {
        $log.info("Initialising location services");
    ...
    */
    ko.utils.registerEventHandler(window, 'popstate', function () {
        if(new hx.Uri(document.location.toString()).toString() !== uri().toString()) {
            updateUri();
    ...
    var currentFragment, nativeHistory, nativePushState, nativeReplaceState;
